...
// PATCH /api/admin/insurance/partners/:id   ✅ FIX (required for enable/disable)
...
// POST /api/admin/insurance/codes/generate  ✅ already used by dashboard page now
...
// PATCH /api/admin/insurance/codes/:id/disable  ✅ FIX (required for Disable button)
 ...
 * INVOICE (TEMP BASIC)
 * ============================
 * Your backend zip does NOT contain job->insurance linkage logic yet,
 * so this returns a minimal usage-based invoice summary using codes.
 *
...
router.get("/invoice", auth, requireAdmin, async (req, res) => {
  try {
    ...
    // Minimal: count codes used in this month (usage.lastUsedAt)
    const usedCodes = await InsuranceCode.find({
      partner: partnerId,
      countryCode,
      "usage.lastUsedAt": { $gte: start, $lt: end },
      "usage.usedCount": { $gt: 0 },
    })
      .select("code usage.lastUsedAt")
      .lean();
    ...
    const invoice = {
      ...
      month,
      currency: "ZAR",
      totalJobs: usedCodes.length,
      totalAmount: 0,
      items: usedCodes.map((c) => ({
        jobId: c.code, // placeholder until job linkage exists
        createdAt: c.usage?.lastUsedAt ? new Date(c.usage.lastUsedAt).toISOString() : start.toISOString(),
